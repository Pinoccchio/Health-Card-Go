...
/**
 * POST /api/admin/super-admins/create
 *
  ...
  try {
    ...
    if (profileError || !profile || profile.role !== 'super_admin') {
      return NextResponse.json(
        { error: 'Forbidden: Only Super Admin can create Super Admins' },
        { status: 403 }
      );
    }
    ...
    if (fetchError || !createdAdmin) {
      ...
      return NextResponse.json(
        { error: 'Super Admin created but failed to fetch details' },
        { status: 500 }
      );
    }
    ...
    await logAuditAction({
      ...
      entityType: AUDIT_ENTITIES.SUPER_ADMIN,
    ...
    console.error('Unexpected error in POST /api/admin/super-admins/create:', error);
    return NextResponse.json(
