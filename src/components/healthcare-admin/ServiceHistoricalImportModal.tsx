...
import { parseServiceAppointmentExcel, validateServiceAppointmentExcelFile } from '@/lib/utils/serviceAppointmentExcelParser';

interface ServiceHistoricalImportModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function ServiceHistoricalImportModal({
  isOpen,
  onClose,
  onImportSuccess,
  serviceId,
  serviceName,
}: ServiceHistoricalImportModalProps) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validate file
    const validationError = validateServiceAppointmentExcelFile(selectedFile);
    if (validationError) {
  ...
  const parseFile = async (selectedFile: File) => {
    ...
    try {
      const result = await parseServiceAppointmentExcel(selectedFile, serviceId);
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await fetch('/api/services/historical/import', {
        method: 'POST',
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Import {serviceName} Appointment Data</h2>
              <p className="text-sm text-gray-600">Upload an Excel file to batch import historical appointment statistics</p>
            </div>
          </div>
          <button
            onClick={handleClose}
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-purple-900 mb-1">Need a template?</h4>
                            ...
                            <div className="bg-white rounded p-3 text-xs space-y-1">
                              {parseResult.validRecords.slice(0, 5).map((record, index) => (
                                <div key={index} className="flex justify-between text-gray-700">
                                  <span>{record.appointment_month}</span>
                                  <span className="font-medium capitalize">{record.status}</span>
                                  <span>{record.appointment_count} appointments</span>
