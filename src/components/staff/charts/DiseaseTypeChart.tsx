...
import { aggregateByDiseaseType, formatNumber, formatPercentage } from '@/lib/utils/chartDataTransformers';
...
export default function DiseaseTypeChart({ data, timeRangeMonths = 24 }: DiseaseTypeChartProps) {
  const chartData = useMemo(() => {
    if (!data || data.length === 0) {
      return null;
    }

    const aggregated = aggregateByDiseaseType(data, timeRangeMonths);
  ...
  const diseaseStats = useMemo(() => {
    ...
    return aggregateByDiseaseType(data, timeRangeMonths);
  }, [data, timeRangeMonths]);
    ...
    <div className="bg-white rounded-lg shadow p-6 border border-gray-200">
        ...
        {diseaseStats.map((disease, index) => (
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <span className="text-gray-600">{formatNumber(disease.count)}</span>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        <p className="text-xs text-gray-500 text-center">
          Showing percentage distribution of all disease types {timeRangeMonths === 'all' ? 'from all time' : `from the last ${timeRangeMonths} months`}
        </p>
      </div>
    </div>
  );
